...
jessie-lite-version = 2017-07-05
docker-tag = raspbian_lite-$(jessie-lite-version)-$(git)

# Build each of the supported images.
build: clean build-jessie
	
build-jessie: pibuilder.patch first_run.patch
	docker build --build-arg JESSIE_LITE_VERSION=$(jessie-lite-version) -t gencore/auto-pi-bakery-jessie:$(docker-tag) .
	docker tag gencore/auto-pi-bakery-jessie:$(docker-tag) gencore/auto-pi-bakery-jessie:latest
